...
<script id="formqlist" type="text/html">
        ...
        <div class="q-content policy">
            {{each itemobj.option_values as lab}}
            <textarea class="form-control" rows="3" placeholder="请填写条款内容" name="{{itemobj.pk_enrol_form_item}}" data-value="{{lab.labeltxt}}" qid="{{itemobj.pk_enrol_form_item}}">{{lab.labeltxt}}</textarea>
            {{/each}}
            <br>
            <input type="checkbox" name="" value="yes" /> 我已阅读并同意参赛条款
        </div>
        {{/if}}
    </li>
    {{/each}}
