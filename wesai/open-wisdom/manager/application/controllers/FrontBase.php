...
abstract class FrontBase extends FRONT_Controller
{
    ...
    private function logUserAction(){
        ...
        if($method == 'post'){
            $params['fk_corp'] = $this->userInfo->pk_corp;
            $params['fk_corp_user'] = $this->userInfo->pk_corp_user;
            $params['controller'] = $this->router->class;
            $params['action'] = $this->router->method;
            $params['url'] = $_SERVER['REQUEST_URI'];
            $params['ext'] = json_encode($this->input->post());
            $this->callInnerApiDiy(API_HOST_OPEN_USER, 'corp_user/action.json', $params, METHOD_POST);
        }
    }
