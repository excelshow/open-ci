    ...
    var itemList = require('./itemList.handlebars');
    var verifyingItemList = require('./verifying_item_list.handlebars');
        ...
        $(document).on("pageInit", "#page-loose", function(e, id, page) {
            ...
            function getVerifyInfo(orderId) {
                ...
                api.ajax_getOrderById(params).done(function(rs) {
                    if (!rs.error) {
                        if(rs.result.fk_corp != rs.pkCorp){
                            $.toast('订单无效', 1000);
                            $.hidePreloader();
                            return;
                        }
                        $.popup('.popup-loose');
                        rs.result.enrol_info.map(function(item, index) {
                            if (item.type == "uploadfile") {
                                item['uploadfile'] = true;
                                item.value = currdomain + '/' + item.value;
                            }
                        })

                        var verifyInfo = verifyingItemList(rs.result);
            ...
            $(document).on('click', '#WicketButton', function() {
                ...
                api.ajax_verifyLoose(params).done(function(rs) {
                    if (!rs.error) {
                        $.toast('检票成功', 1000);
                        setTimeout(function() {
