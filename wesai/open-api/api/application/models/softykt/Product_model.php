...
class Product_model extends DIY_Model {
    ...
    public function productStaticSve($scenicid, $productid){
        ...
        $requests[] = $this->request('api_host_open_contest', 'softykt/product/productStaticSave.json', $param, 'post');
        if(!empty($requests)){
    ...
    public function productModify($param){
        $requests[] = $this->request('api_host_open_contest', 'softykt/product/modify.json', $param, 'post');
        if(!empty($requests)){
            return $this->result($requests);
        }
    ...
    public function create($proInfo, $corp_id, $user_id){
        ...
        $requests[] = $this->request('api_host_open_contest', 'softykt/Contest/add.json', $paramPro, 'post');
        if(!empty($requests)){
            return $this->result($requests);
        }
    }
    ...
    public function get_by_scenicid_productid($scenicid, $productid){
        ...
        $requests[] = $this->request('api_host_open_contest', 'softykt/Contest/get_by_scenicid_productid.json', $param, 'get');
        if(!empty($requests)){
            return $this->result($requests);
        }
    }
